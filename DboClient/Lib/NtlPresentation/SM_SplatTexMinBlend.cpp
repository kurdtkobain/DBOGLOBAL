...
CSM_SplatTexMinBlend::CSM_SplatTexMinBlend(void)
{
}
...
RwBool CSM_SplatTexMinBlend::DestroySelfRetMap()
{
	ISectorMethod::tSELF_RET_MAP_ITER The = m_SelfRetMap.begin();
	while(The != m_SelfRetMap.end())
	{
		CSM_SplatTexMinBlend_RetContainer* pDel = static_cast<CSM_SplatTexMinBlend_RetContainer*>((*The).second);
		NTL_DELETE(pDel);
...
VOID CSM_SplatTexMinBlend::RemoveSplatTex(RwInt32 _FieldIdx, RwInt32 _SectorIdx, RwBool _Indices2Del[dNTL_SPLAT_LAYER_NUM])
{
	...
	if(!fopen_s(&pNewFieldFile, dGET_WORLD_PARAM()->WorldChar64Buf, "wb"))
	{
		for(RwInt32 i = 0; i < 4; ++i)
		{
			...
			{
				pPosCurFileMem = GetNtlWorldSectorInfo()->SectorMaterialSaveIntoFileFromFileMemSplatTexMinBlend(pNewFieldFile, pPosCurFileMem, _Indices2Del);
...
RwBool CSM_SplatTexMinBlend::Execute(eSECTOR_METHOD_EXECUTION_TYPE_INDEX _SMETI)
{
	...
	while(The != m_SelfRetMap.end())
	{
		...
		CSM_SplatTexMinBlend_RetContainer*	pCurRetContainer		= static_cast<CSM_SplatTexMinBlend_RetContainer*>((*The).second);
		RwBool								SplatLayerIndices2Del[dNTL_SPLAT_LAYER_NUM];

		for(RwInt32 i = 0; i < dNTL_SPLAT_LAYER_NUM; ++i)
		{
			SplatLayerIndices2Del[i] = ((pCurRetContainer->m_AverageAlphaDensity[i] < dMIN_BLEND_DENSITY) ? TRUE : FALSE);
		}

		RemoveSplatTex(CurFieldIdx, CurSectorIdx, SplatLayerIndices2Del);
...
RwBool CSM_SplatTexMinBlend::CheckSector(BYTE* _pFileMemPos)
{
	/*
	...
	for(RwInt32 i = 0; i < dNTL_SPLAT_LAYER_NUM; ++i)
	{
		...
		if(BoolFlag)
		{
			...
			if(BoolFlag)
			{
				...
				if(m_RetContainer.m_AverageAlphaDensity[i] < dMIN_BLEND_DENSITY)
				{
					Ret = TRUE;
				}
			}
...
VOID* CSM_SplatTexMinBlend::GetAnalysis()
{
	CSM_SplatTexMinBlend_RetContainer* _pRet = NTL_NEW CSM_SplatTexMinBlend_RetContainer;
...
RwBool CSM_SplatTexMinBlend::Analyze(DWORD _SectorIdx)
{
	CNtlWorldFieldManager*	pMgr		= GetSceneManager()->GetWorld()->GetWorldFieldMgr();
	CNtlWorldField*			pFields		= const_cast<CNtlWorldField*>(pMgr->GetFields());
	RwV3d					Pos;
...
RwBool CSM_SplatTexMinBlend::Report()
{
	CNtlWorldFieldManager*	pMgr	= GetSceneManager()->GetWorld()->GetWorldFieldMgr();
	CNtlWorldField*			pFields	= const_cast<CNtlWorldField*>(pMgr->GetFields());
	RwInt32					CurSectorIdx;
	...
	if(!fopen_s(&pFile, "SM_SPLAT_TEX_MIN_BLEND.rpt", "wt"))
	{
...
VOID CSM_SplatTexMinBlend::RemoveSplatTex(RwInt32 _FieldIdx, RwInt32 _SectorIdx, RwBool _Indices2Del[dNTL_SPLAT_LAYER_NUM])
{
	...
	if(!fopen_s(&pNewFieldFile, dGET_WORLD_PARAM()->WorldChar64Buf, "wb"))
	{
		for(RwInt32 i = 0; i < 4; ++i)
		{
			...
			{
				pPosCurFileMem = OnSM_SplatTexMinBlend(pPosCurFileMem, pNewFieldFile, _Indices2Del);
