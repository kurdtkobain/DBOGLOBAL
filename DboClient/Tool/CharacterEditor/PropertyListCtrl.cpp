...
{	
	...
	if(pPropertyItem != NULL)
	{
		...
		pPropertyItem->SetData(m_pWndInPlaceControl);
		m_pPropertyItemManager->OnLButtonDown(pPropertyItem, this, nItem);
	}
}
...
void CPropertyItemManagerAdaptable::CheckState(CPropertyListCtrl* pWndPropertyListCtrl, int nFromControlIndex, BYTE nFromPropertyItemID)
{
	...
	while(pos != NULL)
	{
		...
		while(posTab != NULL)
		{
			...
			{
				// Naiel si poloku od ktorej by mali nasledova?zmeny
