...
//
TCHAR MSJExceptionHandler::m_szLogFileName[EXCEPT_MAX_DIR_PATH];
LPTOP_LEVEL_EXCEPTION_FILTER MSJExceptionHandler::m_previousFilter;
FILE* MSJExceptionHandler::m_pReportFile;
MSJExceptionHandler::APPLICATION_FILTER MSJExceptionHandler::m_pApplicationFilter = NULL;
...
MSJExceptionHandler::MSJExceptionHandler( )
{
    ...
    GetModuleFileName( 0, m_szLogFileName, EXCEPT_MAX_DIR_PATH );
...
//=====================================
// Lets user Add Filter of Application 
//=====================================
void MSJExceptionHandler::SetApplicationFilter( APPLICATION_FILTER pApplicationFilter )
{
	m_pApplicationFilter = pApplicationFilter;
}
...
void MSJExceptionHandler::SetLogFileName( PTSTR pszLogFileName )
{
    ...
    _tcscpy_s( m_szLogFileName, EXCEPT_MAX_DIR_PATH, pszLogFileName );
#else
...
LONG WINAPI MSJExceptionHandler::MSJUnhandledExceptionFilter(PEXCEPTION_POINTERS pExceptionInfo )
{
	BOOL bResult = FALSE;
#if defined(_MSC_VER) && (_MSC_VER >= 1400)
    if(fopen_s(&m_pReportFile, m_szLogFileName, "w+") == 0)
		bResult = TRUE;
#else
	m_pReportFile = fopen( m_szLogFileName, "w+");
    ...
    if ( bResult )
    {
        fseek( m_pReportFile, 0, SEEK_END  );
    ...
    PEXCEPTION_POINTERS pExceptionInfo )
{
    ...
    TCHAR szFaultingModule[EXCEPT_MAX_DIR_PATH];
...
void MSJExceptionHandler::IntelStackWalk( PCONTEXT pContext )
{
    ...
    do
    {
        TCHAR szModule[EXCEPT_MAX_DIR_PATH] = _T("");
...
int __cdecl MSJExceptionHandler::_tprintf(const TCHAR * format, ...)
{
    ...
    fwrite( szBuff, sizeof(char), retValue * sizeof(TCHAR), m_pReportFile);
