...
void cObstacleRenderInfo::BuildVertexBuffer()
{
	if( _vertexBuffer == NULL )
	{
		...
		sprintf_s( buf, 256, "Vertex Buffer create start count : %d", (int)_obstacleElements.size() );
		sprintf_s( buf2, 256, "BufferSize : %d byte", sizeof(LINEVERTEX) * (int)_obstacleElements.size() );
		...
		if(result != D3D_OK)
		{
			...
			nSingletons::pathEngineI().getErrorHandler()->handle(buf2, "Create Vertec Buffer Failed", NULL );
			_vertexBuffer = 0;
			return;
		}
