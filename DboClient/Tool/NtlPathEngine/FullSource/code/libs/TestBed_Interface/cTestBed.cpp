...
void cTestBed::setMesh(iMesh *mesh)
{
    ...
    if(_mesh->getTestBedUserData() == 0)
    {
        ...
        meshUserData()._meshRenderInfo->build(_mesh->ref3D(),gip, meshUserData()._edgeRenderInfo);
		meshUserData()._edgeRenderInfo->BuildVertexBuffer();
		...
		meshUserData()._obstacleRenderInfo->Build( *baseCircuits, meshUserData()._baseCircuitsRecorder, _mesh->refTranslation());
		meshUserData()._obstacleRenderInfo->BuildVertexBuffer();
    }
...
cTestBed::drawMeshEdges()
{
    if(_mesh)
    {
		if( meshUserData()._edgeRenderInfo )
		{
			meshUserData()._edgeRenderInfo->Render();
		}
...
//            printText8by16(10,10,buffer);
    ...
    {
        setColour("white");
        Draw3DCross(gip,g_target_position,400.0f);
    }
...
cTestBed::drawBurntInObstacles()
{
	...
	if( meshUserData()._baseCircuitsRecorder )
	{
		meshUserData()._obstacleRenderInfo->Render();
	}
}
...
void* cTestBed::getWindowHandle()
{
	return (void*)nSingletons::testBedHandle();
}